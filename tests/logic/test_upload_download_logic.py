...
def test_upload_file_with_signed_url(caplog):
     ...
     with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        when(upload_download_logic.requests).put(test_signed_url, ...).thenReturn(mock_response)

        # Act
        with caplog.at_level(logging.DEBUG):
            upload_download_logic.upload_file_with_signed_url(test_local_file_path, test_signed_url)
